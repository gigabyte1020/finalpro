...
$fd1 = mysqli_query($db_connection,"SELECT * FROM rent_feed where rent_id = '$rid'");
...
while($row = mysqli_fetch_array($fd1))
  ...
  {$t1= $row['fdbk_id'];
    ?> 
                    <ul><li>    <form method="POST">           <?php echo $row['fdbk_det']; $t1= $row['fdbk_id'];?>
                                <input style="" type="submit" value="Delete" name="del"><br></form></li></ul>
                            <?php
                            
                }
if(isset($_POST['del']) )
{
    $sql1= "DELETE FROM `rent_feed` WHERE fdbk_id='$t1'";
